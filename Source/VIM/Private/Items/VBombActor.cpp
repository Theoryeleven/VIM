...
#include "VIM.h"
#include "VBombActor.h"


AVBombActor::AVBombActor(const class FObjectInitializer& ObjectInitializer)
...
void AVBombActor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void AVBombActor::OnUsed(APawn* InstigatorPawn)
{
	if (bIsFuzeActive)
	{
		return;
	...
	GetWorldTimerManager().SetTimer(FuzeTimerHandle, this, &AVBombActor::Explode, MaxFuzeTime, false);

}


void AVBombActor::Explode()
{
	if (bExploded)
	{
		return;
...
void AVBombActor::SimulateFuzeFX_Implementation()
{
	if (FuzeSound)
	{
...
void AVBombActor::SimulateExplosion_Implementation()
{
...
float AVBombActor::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, class AActor* DamageCauser)
{
	Explode();
